...
==== Get the response from the exception instance

Some exception classes like https://docs.spring.io/spring-framework/docs/current/javadoc-api/org/springframework/web/server/ResponseStatusException.html[`ResponseStatusException`] or https://docs.spring.io/spring-framework/docs/current/javadoc-api/org/springframework/web/client/RestClientResponseException.html[`RestClientResponseException`] contain the wanted response status. The library will automatically use that response status in the returned response.
...
[TIP]
====
This mechanism is extensible. If you have an exception class that contains the response status, then you can create a bean that implements `io.github.wimdeblauwe.errorhandlingspringbootstarter.mapper.HttpResponseStatusFromExceptionMapper` to have the library automatically extract the response status from the exception.
...
@Component
class CustomHttpResponseStatusFromExceptionMapper implements HttpResponseStatusFromExceptionMapper {
